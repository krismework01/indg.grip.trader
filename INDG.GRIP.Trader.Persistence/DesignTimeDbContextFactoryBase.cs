        ...
        : IDesignTimeDbContextFactory<TContext> where TContext : DbContext
    {
        ...
        public TContext CreateDbContext(string[] args)
        {
            var basePath = Directory.GetCurrentDirectory() +
                           string.Format("{0}..{0}INDG.GRIP.Trader.Bootstrapper", Path.DirectorySeparatorChar);
        ...
        private TContext Create(string basePath, string environmentName)
        {
            ...
            var connectionString = configuration
                                       .GetSection($"ConnectionStrings:{ConnectionStringName}:connectionString")
                                       .Value +
                                   $"Database={configuration.GetSection($"ConnectionStrings:{ConnectionStringName}:Database").Value};";
